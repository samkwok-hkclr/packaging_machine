...
inline uint8_t _get_od_u8(uint16_t index)
{
	uint8_t val;
...
inline uint16_t _get_od_u16(uint16_t index)
{
	uint16_t val;
...
inline void _set_od_u8(uint16_t index, uint8_t val)
{
	if (OD_set_u8(OD_find(OD, index), 0x00, val, false) != ODR_OK)
		show_err_LED();
}

inline void _set_od_u16(uint16_t index, uint16_t val)
{
